<template>
	<div class="post-tags post__content">
			<g-link class="post-tags__icon"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
			...
			<g-link class="post-tags__link" v-for="(tag, index) in post.tags" :key="tag.id" :to="tag.path">
				{{ tag.title }}
			</g-link>
	</div>
</template>
...
.post-tags {
	...
	&__link {
		...
		// color: var(--color-white) !important; //Todo: remove important;
		padding: .5rem .6rem;
	...
	//   	color: var(--color-primary) !important; //Todo: remove important;
	//   }
}
</style>
